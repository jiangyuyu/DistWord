...
public class MapLDADataset {
	...
	public MapLDADataset(int M, Dictionary globalDict){
		assert (M > 0);
		localDict = new Dictionary();	
		this.M = M;
		this.V = 0;
	 ...
	 */
	public static MapLDADataset readDataSet(String filename){
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(
					new FileInputStream(filename), "UTF-8"));
			
			MapLDADataset data = readDataSet(reader);
	 ...
	 * @return dataset if success and null otherwise
	 */
	public static MapLDADataset readDataSet(String filename, Dictionary dict){
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(
					new FileInputStream(filename), "UTF-8"));
			MapLDADataset data = readDataSet(reader, dict);
	 ...
	 *  @return dataset if success and null otherwise
	 */
	public static MapLDADataset readDataSet(BufferedReader reader){
		try {
			...
			MapLDADataset data = new MapLDADataset(M);
			for (int i = 0; i < M; ++i){
				line = reader.readLine();
	 ...
	 * @return dataset if success and null otherwise
	 */
	public static MapLDADataset readDataSet(BufferedReader reader, Dictionary dict){
		try {
			//read number of document
			String line;
			line = reader.readLine();
			...
			MapLDADataset data = new MapLDADataset(M, dict);
			for (int i = 0; i < M; ++i){
				line = reader.readLine();
	 ...
	 * @return dataset if success and null otherwise
	 */
	public static MapLDADataset readDataSet(String [] strs){
		MapLDADataset data = new MapLDADataset(strs.length);
	 ...
	 * @return dataset if success and null otherwise
	 */
	public static MapLDADataset readDataSet(String [] strs, Dictionary dict){
		//System.out.println("readDataset...");
		MapLDADataset data = new MapLDADataset(strs.length, dict);
