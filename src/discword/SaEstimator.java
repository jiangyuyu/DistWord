...
public class SaEstimator extends Estimator {
    protected SaModel trnModel;
    CircularZ cz = null;
    ...
    public void estimate(){
        ...
        double oldPerplexity = 0.0, newPerplexity = 0.0;
        double[] newTK = new double[trnModel.V];
        for (trnModel.liter = lastIter + 1; trnModel.liter < option.niters + lastIter; trnModel.liter++){           
            ...
            if (trnModel.liter % updateInterval == 0) {
                // Compute old perplexity and Save old TK locally
                oldPerplexity = trnModel.perplexity();
                
                // Compute new TK and perplexity.
                Util.copy(trnModel.TK, newTK);
                trnModel.computeWordInfoRate(newTK);
                newPerplexity = newPerplexity(newTK);
                
                if (newPerplexity <= oldPerplexity || Util.random() < Math.exp((1.0 - newPerplexity / oldPerplexity) * trnModel.liter)) {
                    Util.copy(newTK, trnModel.TK);
                    System.out.println("sum of word rate: " + Util.sum(trnModel.TK));
                    System.out.println(String.format("new word rate is applied: %d - old-pplt=%.6f new-pplt = %.6f", 
                            trnModel.liter, oldPerplexity, newPerplexity));
                } else {
                    System.out.println(String.format("new word rate is not applied: %d - old-pplt=%.6f new-pplt = %.6f", 
                            trnModel.liter, oldPerplexity, newPerplexity));
                }
                
                
                double sample_sum = Util.sum(trnModel.ndsum);
                double perplexity = oldPerplexity;
    ...
    public double newPerplexity(double[] TK) {
        int K = trnModel.K, M = trnModel.M, V = trnModel.V;
        
        double[][] nw = new double[V][K];
        double[][] nd = new double[M][K];
        double[] nwsum = new double[K];
        double[] ndsum = new double[M];
        double total = 0.0;
        for (int m=0; m<M; m++) {
            for (int n=0; n<trnModel.data.docs[m].length; n++) {
                int t = trnModel.z[m].get(n);
                int w = trnModel.data.docs[m].words[n];
                nw[w][t] += TK[w];
                nwsum[t] += TK[w];
                nd[m][t] += TK[w];
        ...
        return trnModel.perplexity(nd, nw, ndsum, nwsum);
    }
